...
**2025 年我尝试使用主路由部署 WG，记录一下不同的地方：**

1.openwrt 界面已经支持密钥创建，不需要命令行创建了

2.可在 openwrt 上面直接创建对端配置并扫码获取

3.添加端口转发规则时 WAN 口端口不要使用 WG 本身监听端口，转发到路由器 LAN ip 上 而不是 WG ip

4.将 WG 加入 VPN 或者新建防火墙区域后，设置该区域到 LAN 的转发规则，同时该区域内出入转都设置为允许（不需要开启 IP 动态伪装）
:::
